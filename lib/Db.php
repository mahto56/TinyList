...
class Db{
    ...
    {   $query = "SELECT l.list_id,l.list_name from tasklists l,users u WHERE l.owner_id=u.user_id AND u.username='".$_SESSION['username']."'";
        $data = $this->mysqli->query($query) or die("ERROR: Could not get tasklists" . mysqli_error($this->mysqli));
        if($data->num_rows > 0){
            return $data;
        }
        else{
